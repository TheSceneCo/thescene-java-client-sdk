...
import nz.co.thescene.client.traverson.Rels.Rel;
...
public class Traverson {
	...
	private static final RestOperations createDefaultTemplate(List<MediaType> mediaTypes) {
		...
		addRequestInterceptors(template);
	...
	// JD: Add request interceptor for logging
	private static void addRequestInterceptors(RestTemplate template) {
	...
	private <T> HttpEntity<?> prepareRequest(HttpHeaders headers, T payload) { 
		HttpHeaders toSend = new HttpHeaders();
		toSend.putAll(headers);
	...
	private <T> HttpEntity<?> prepareMultipartRequest(HttpHeaders headers, T payload) { 
		
		HttpHeaders toSend = new HttpHeaders();
		toSend.putAll(headers);
		toSend.setContentType(MediaType.MULTIPART_FORM_DATA);
	...
	public class TraversalBuilder {
		...
		public <T> T toObject(HttpMethod method, Class<T> type) {
			return operations.exchange(traverseToExpandedFinalUrl(), method, prepareRequest(headers), type).getBody();
		}
		
		public <T, D> T toObject(HttpMethod method, D payload, Class<T> type) {
			HttpEntity<?> prepareRequest = prepareRequest(headers, payload);
			return operations.exchange(traverseToExpandedFinalUrl(), method, prepareRequest, type).getBody();
		}
		
		public <T, D> T toObject(HttpMethod method, List<D> payload, Class<T> type) { 
			HttpEntity<?> prepareRequest = prepareMultipartRequest(headers, payload);
			return operations.exchange(traverseToExpandedFinalUrl(), method, prepareRequest, type).getBody();
		}
		
		
		public <T, D> T toObject(HttpMethod method, D payload, ParameterizedTypeReference<T> type) { 
			HttpEntity<?> prepareRequest = prepareRequest(headers, payload);
			return operations.exchange(traverseToExpandedFinalUrl(),  method, prepareRequest, type).getBody();
		}
